...
async def get_result(task_id: str) -> SearchResultResponse:
    ...
    if not batch_results:
        return SearchResultResponse(
            ...
            vendors_failed = vendors_failed,
            error          = "All vendor workers failed. No results available.",
        )

    # Load delivery rules from DB (falls back to hardcoded defaults)
    delivery_rules = await _load_delivery_rules()

    # Run Smart-Split
    t0          = time.monotonic()
    smart_split = compute_smart_split(
        task_id             = task_id,
        batch_results       = batch_results,
        medicine_quantities = medicine_quantities,
        delivery_rules      = delivery_rules,
    )
    duration_ms = round((time.monotonic() - t0) * 1000, 1)

    logger.info(
        "smart_split_complete",
        grand_total=smart_split.grand_total,
        total_saving=smart_split.total_saving,
        split_advantage=smart_split.split_advantage,
        duration_ms=duration_ms,
    )

    # Cache in Redis
    cache_key = meta_raw.get("cache_key")
    if cache_key:
        await r.set(f"pb:cache:{cache_key}", smart_split.model_dump_json(), ex=CACHE_TTL_S)
    ...
    try:
        ...
        async with db_session() as db:
            ...
            for batch in batch_results:
                await s_repo.upsert_vendor_result(
                    search_id   = task_id,
            ...
            pincode = meta_raw.get("pincode", "000000")
            rows = [
                {
                ...
                for batch in batch_results for item in batch.results
            ]
            if rows:
                p_repo = VendorPriceHistoryRepository(db)
                await p_repo.bulk_insert(rows)
    except Exception as exc:
        logger.warning("db_persist_result_failed", task_id=task_id, error=str(exc))
...
async def user_search_history(
    ...
    repo    = SearchRepository(db)
    results = await repo.get_user_history(token.sub, limit=limit, offset=offset)
    return {
        "searches": [
            {
                "search_id"   : s.id,
...
async def _write_task_meta(
    ...
    mapping   = {
        "task_id"    : task_id,
        "vendors"    : json.dumps([v.value for v in req.vendors]),
        "medicines"  : json.dumps([m.model_dump() for m in req.medicines]),
        "pincode"    : req.pincode,
        "queued_at"  : queued_at.isoformat(),
    ...
    await r.hset(f"pb:task:{task_id}:meta", mapping=mapping)
    await r.expire(f"pb:task:{task_id}:meta", RESULT_TTL_S)


def _build_cache_key(req: SearchRequest) -> str:
    fingerprint = {
        "medicines": sorted(
            [{"name": req.pincode and m.name.strip().lower(), "qty": m.quantity}
             for m in req.medicines],
            key=lambda x: x["name"],
        ),
        "pincode"       : req.pincode,
        "prefer_generic": req.prefer_generic,
        "vendors"       : sorted([v.value for v in req.vendors]),
    }
